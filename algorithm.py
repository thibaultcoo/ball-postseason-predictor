...
class nba_algorithm:
    ...
    def __init__(self, starting_year = 2022, ending_year = 2022, type_stat = ["W","L","ORtg","DRtg","NRtg","Pace","FTr","3PAr"]):
    ...
    def main_nba(self): # for now there is only one iteration, but we will want a loop to isolate the best model from many combinations
        ...
        transformed_set = data_transformator(entire_set = entire_set, type_stat = self.type_stat).nba_data_transformator() # this function is supposed to create a random non-linear structure between the variables
        score = data_regressor(whole_set_x = transformed_set[0], whole_set_y = transformed_set[1]).nba_data_regressor() # simple linear regression

        return score

print(nba_algorithm().main_nba())

#print(data_regressor(whole_set = data_extractor(year="2001", step="leagues", type="game", export="no").nba_data_extractor()).nba_data_regressor())