    ...
    def main_nba(self):
        ...
        if self.new_structure == "no":
            ...
            with open(address) as f:
                ...
                self.starting_year = int(file[1])
                self.ending_year = int(file[2])
                self.type_stat = ast.literal_eval(file[5])
        ...
        model_storer(structure=transformed_set[2], starting_year=self.starting_year, ending_year=self.ending_year, iter=iter, elapsed_time=elapsed_time,
                     input_variables=self.type_stat, score=score).nba_model_storer()
